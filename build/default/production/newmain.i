...
void main(void) {
    ...
    DESIRED_TEMP = (EEPROM_Read(0x00))*10;
    DESIRED_TEMP += (EEPROM_Read(0x01)) ;


    while(1){
        if( (RB2 == 0 || RB1 == 0) && MODE == 1){
            while (RB2 == 0);
            MODE = 0;
        ...
        if (MODE == 0 && RB1 == 0 && DESIRED_TEMP != 35){
            while (RB1 == 0);
            DESIRED_TEMP = DESIRED_TEMP - 5;
            MODE_COUNTER = 0;
            EEPROM_Write(0X00 ,DESIRED_TEMP/10);
            EEPROM_Write(0X01,DESIRED_TEMP - (DESIRED_TEMP/10)*10);
            continue;
        }
        if (MODE == 0 && RB2 == 0 && DESIRED_TEMP != 75 ){
            while (RB2 == 0);
            DESIRED_TEMP = DESIRED_TEMP + 5;
            MODE_COUNTER = 0;
            EEPROM_Write(0x00,DESIRED_TEMP/10);
        ...
        if ( MODE == 1 ){
             if( ADC_TEMP > (DESIRED_TEMP+5) && RC2 == 0){
                LCD_SendCommand(0x01);
                LCD_Write_String("COOLING");
                RC5=0;
                RC2=1;
                continue;
            }
             else if( ADC_TEMP <(DESIRED_TEMP-5) && RC5 == 0){
                LCD_SendCommand(0x01);
        ...
        if (MODE == 1){
         SEG_Write(ADC_TEMP/10);
         _delay((unsigned long)((80)*(8000000/4000000.0)));
         SEG_Write1(ADC_TEMP - (ADC_TEMP/10)*10);
        }
        else if (MODE == 0){
            SEG_Write(DESIRED_TEMP/10);
            SEG_Write1(DESIRED_TEMP - (DESIRED_TEMP/10)*10 );
        }
    }
          return;
}
    ...
    if (TMR1IF){
        ...
        if (MODE_COUNTER == (156/5 && RC5 == 1 )){
            ...
            if(ADC_counter == 60){
                ADC_counter=0;
                ADC_TEMP= ADC_Read();
                ADC_READINGS[READINGS_INDEX] = ADC_TEMP;
                READINGS_INDEX = (READINGS_INDEX%+1) % 10 ;
                calc_average();
            }
        }
        TMR1IF = 0;
